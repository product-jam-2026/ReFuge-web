...
//       {/* -----------------------------
//           Everything below: keep your UI,
//           but make sure it reads/writes from
//           draft/extras via update/setExtras
//          ----------------------------- */}

//       {/* Example fields (replace with your real Step-4 UI) */}
...
//       {/* If you use add child row in this step, just call addChildRow() */}
//       {/* <button type="button" onClick={addChildRow} className={styles.secondaryBtn}>
//         + הוסף ילד/ה נוסף/ת
//       </button> */}
...
//       {/* Debug */}
//       {/* <pre>{JSON.stringify({ draft, extras }, null, 2)}</pre> */}
//     </main>
//   );
// }
...
export default function Step4Page() {
  ...
  const {
    ...
    // ✅ saving like the other wizard
    saveNow,
    saveStatus,
    saveError,
  } = useWizard();

  // ✅ mark current step so the homepage can resume correctly
  useEffect(() => {
  ...
  // ✅ keep instanceId while navigating forward
  const nextUrl = instanceId
    ? `./step-5?instanceId=${encodeURIComponent(instanceId)}`
    : "./step-5";

  async function onSaveAndExit() {
    const id = await saveNow(); // creates/updates the instance + keeps instanceId in provider
    if (id) {
  ...
  return (
    <main className={styles.page}>
      ...
      {/* -----------------------------
          Keep your Step-4 UI here.
          Make sure every field writes to:
          - draft via update/updateChild
          - extras via setExtras
         ----------------------------- */}
