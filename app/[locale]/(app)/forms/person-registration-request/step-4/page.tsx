...
export default function Step4() {
  ...
  async function uploadPdf(outBytes: Uint8Array, formInstanceId: string, pdfTitle: string) {
    const supabase = createClient();
    ...
    const path = `${user.id}/person-registration-request/${formInstanceId}/${fileName}`;
  ...
  async function onGenerateAndFinish() {
    ...
    // ✅ use WizardProvider saving (consistent)
    const savedId = instanceId ?? (await saveNow());
    if (!savedId) throw new Error("Failed to save draft before generating PDF");

    await uploadPdf(outBytes, savedId, pdfTitle);

    // keep instanceId when going to review
    router.push(`./review?instanceId=${encodeURIComponent(savedId)}`);
  }
  ...
  return (
    <main className={styles.page}>
      ...
      <Field label="اسم النموذج   שם הטופס">
        <input
          value={(extras as any).formTitle}
          onChange={(e) => setExtras((p: any) => ({ ...p, formTitle: e.target.value }))}
          className={styles.input}
          placeholder="שם לזיהוי ברשימות"
        />
      </Field>
